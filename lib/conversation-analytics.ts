export interface SessionData {
  ...
  export class ConversationAnalytics {
    ...
    startSession(userId: string, companionId: string, topic: string, totalSteps: number): string {
    ...
    recordStepCompletion(sessionId: string, completion: StepCompletion) {
      ...
      session.averageAccuracy = completions.reduce((sum, c) => sum + c.accuracyScore, 0) / completions.length
      session.averageResponseTime = completions.reduce((sum, c) => sum + c.responseTime, 0) / completions.length
  
      console.log(`📈 Step ${completion.stepNumber} completed with ${completion.accuracyScore}% accuracy`)
    }
    ...
    private generateInsights(session: SessionData, completions: StepCompletion[]): SessionInsights {
      ...
      if (completions.length > 5) {
        ...
        const firstHalfAvg = firstHalf.reduce((sum, c) => sum + c.accuracyScore, 0) / firstHalf.length
        const secondHalfAvg = secondHalf.reduce((sum, c) => sum + c.accuracyScore, 0) / secondHalf.length
    ...
    getUserProgress(userId: string): any {
      ...
      const averageScore = userSessions.reduce((sum, s) => sum + s.averageAccuracy, 0) / totalSessions
