...
export function ResumeCard({ companion }: ResumeCardProps) {
  ...
  return (
    <div>
      <h2 className="flex items-center gap-2 mb-4 text-2xl font-bold text-black-100">
        <History className="w-6 h-6" />
        Continue Where You Left Off
      </h2>
      <Card
        className="overflow-hidden transition-all border-2 hover:shadow-lg"
        style={{ borderColor: subjectColor }}
      >
        <div className="flex flex-col md:flex-row">
          {/* Optional: Add an image for the companion */}
          {/* <div className="bg-gray-200 md:w-1/3">
            <img src={companion.image_url} alt={companion.name} className="object-cover w-full h-full" />
          </div> */}
          ...
          <div className="flex flex-col justify-between flex-1 p-6">
            ...
            <div className="mt-6">
              <Link href={`/companion-library/conversation/${companion.id}`} passHref>
